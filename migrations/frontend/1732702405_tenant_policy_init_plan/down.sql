...
CREATE POLICY tenant_isolation_policy_new ON gitserver_repos
    USING (
...
ALTER POLICY tenant_isolation_policy_new ON gitserver_repos RENAME TO gitserver_repos_isolation_policy;
COMMIT AND CHAIN;

CREATE POLICY tenant_isolation_policy_new ON repo
    USING (
    (current_setting('app.current_tenant'::text) = 'zoekttenant') OR
    tenant_id = (NULLIF(current_setting('app.current_tenant'::text), 'zoekttenant')::integer)
    );
...
ALTER POLICY tenant_isolation_policy_new ON repo RENAME TO repo_isolation_policy;
COMMIT AND CHAIN;

CREATE POLICY tenant_isolation_policy_new ON zoekt_repos
    USING (
    (current_setting('app.current_tenant'::text) = 'zoekttenant') OR
    tenant_id = (NULLIF(current_setting('app.current_tenant'::text), 'zoekttenant')::integer)
    );
...
ALTER POLICY tenant_isolation_policy_new ON zoekt_repos RENAME TO zoekt_repos_isolation_policy;
COMMIT AND CHAIN;
